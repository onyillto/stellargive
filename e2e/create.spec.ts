      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          result: {
            ...
            results: [
              { xdr: "AAAAEAAAAAEAAAACAAAADwAAAAV0aXRsZQAAAAAAAA0AAAA=" },
            ],
            latestLedger: 1000,
          },
        }),
...
test.describe("Create Campaign flow", () => {
  ...
  test("submits a valid campaign and navigates to the detail page", async ({
    page,
  }) => {
    await page.goto("/create");

    await expect(
      page.getByRole("heading", { name: /Create a New Campaign/i }),
    ).toBeVisible();
  ...
  test("shows validation feedback for an invalid target amount", async ({
    page,
  }) => {
    await page.goto("/create");
